...
# Modified version using NixlConnector and toy_proxy_server
...
launch_disagg_prefill() {
  model="meta-llama/Meta-Llama-3.1-8B-Instruct"
  # disagg prefill with NixlConnector
  CUDA_VISIBLE_DEVICES=0 python3 \
    -m vllm.entrypoints.openai.api_server \
    --model $model \
    --port 8100 \
  ...
  wait_for_server 8100
  wait_for_server 8200
  # Use toy_proxy_server for better visibility and control over KV cache transfer
...
main() {
  ...
  launch_disagg_prefill
  for qps in 2 4 6 8; do
  benchmark $qps $default_output_len disagg_prefill_nixl
  done
  kill_gpu_processes
